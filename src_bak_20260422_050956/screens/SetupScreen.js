...
export default function SetupScreen({ route, navigation }) {
  ...
  console.log('[SetupScreen] isHost:', isHost, 'gameCode:', gameCode);
  const { room, updateRoom } = useRoomSync(gameCode);
  const { getPlayerId } = useFirebaseRoom();
  const getPlayerIdRef = React.useRef(getPlayerId);
  ...
  React.useEffect(() => {
    getPlayerIdRef.current = getPlayerId;
  }, [getPlayerId]);

  React.useEffect(() => {
    if (isHost) return;
    if (!roomChallengeKey) return;
    if (redirectedToRoleRef.current) return;
    console.log('[EFFECT RUN]', 'SetupScreen', 'non-host redirect', {
      isHost,
      roomChallengeKey,
    });
    redirectedToRoleRef.current = true;
    ...
    const me = playerId;
    (async () => {
      const resolvedMe = me || (await getPlayerIdRef.current());
