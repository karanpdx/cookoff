...
export default function RoleAssignmentScreen({ route, navigation }) {
  ...
  const rolesAssignedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    rolesAssignedRef.current = false;
  }, [gameCode]);

  useEffect(() => {
    if (!isHost || roleAssignedOnce || !room?.players?.length) return;
    if (rolesAssignedRef.current) return;
    const allPlayersHaveRoles = room.players.every((p) => p.role && p.role !== 'pending');
    if (allPlayersHaveRoles) {
      rolesAssignedRef.current = true;
      setRoleAssignedOnce(true);
      return;
    }
    rolesAssignedRef.current = true;
