...
export default function SabotageScreen({ route, navigation }) {
  ...
  const seededSabotageRef = useRef(false);
  const hasRoom = Boolean(room);
  const sabotageCardsCount = room?.sabotageCards?.length ?? 0;
  const isHostPlayer = Boolean((room?.players || []).find((p) => p.id === selfId)?.isHost);
  ...
  useEffect(() => {
    console.log('[EFFECT RUN]', 'SabotageScreen', 'seed sabotage cards', {
      hasRoom,
      sabotageCardsCount,
      isHostPlayer,
      seeded: seededSabotageRef.current,
    });
    if (!hasRoom || sabotageCardsCount > 0 || !isHostPlayer || seededSabotageRef.current) return;
    seededSabotageRef.current = true;
    const seedCards = CARDS.map((c) => ({ ...c, claimedBy: null }));
    console.log('[SabotageScreen] host one-time sabotage seed');
    updateRoom({ sabotageCards: seedCards }).catch(() => {
      seededSabotageRef.current = false;
    });
  }, [hasRoom, sabotageCardsCount, isHostPlayer, updateRoom]);
