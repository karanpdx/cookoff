...
export default function CookingChallengeScreen({ route, navigation }) {
  ...
  const hasRoomRecipe = Boolean(room?.recipe);
  const challengeKey = `${room?.challenge?.createdAt || 'local'}|${room?.challenge?.cuisineType || cuisineType || 'any'}`;
  ...
  const cuisineTypeRef = useRef(cuisineType);
  const budgetRef = useRef(budget);
  const modifiersRef = useRef(modifiers);
  const skillLevelRef = useRef(skillLevel);
  const initialTotalSecondsRef = useRef(initialTotalSeconds);
  ...
  cuisineTypeRef.current = cuisineType;
  ...
  useEffect(() => {
    ...
    console.log('[EFFECT RUN]', 'CookingChallengeScreen', 'sync power inventory', {
  ...
  useEffect(() => {
    ...
    if (hasGeneratedRecipeRef.current === challengeKey) return;
    hasGeneratedRecipeRef.current = challengeKey;
    console.log('[CookingChallengeScreen] host one-time recipe seed for challengeKey:', challengeKey);
    let cancelled = false;
    ...
    const callClaude = async (retry = false) => {
      ...
      const currentSkillLevel = skillLevelRef.current;
      const currentInitialTotalSeconds = initialTotalSecondsRef.current;
          ...
          messages: [
            {
              ...
              content: buildRecipeChefPrompt({
                userDish: currentCuisineType || 'chef choice',
                cuisineType: currentCuisineType || 'Any',
                budget: currentBudget,
                cookTimeTarget: Math.max(5, Math.round(currentInitialTotalSeconds / 60)),
                modifiers: currentModifiers,
                skillLevel: currentSkillLevel || 'Intermediate',
                retry,
              }),
            },
          ],
  ...
  const handleDone = useCallback(async () => {
    ...
    if (roomPresent && isHostPlayer) {
      try {
        console.log('[CookingChallengeScreen] host one-time status change: cooking -> voting');
